...
*******************************************************************************/
extern unsigned char g_pucBuffer0[];
extern unsigned char g_pucBuffer1[];
//*****************************************************************************
//
...
{
    {0x0b, 600, 350, 650, 400}, /* Left*/
    {0x07, 700, 350, 750, 400}, /* Right */
    {0x0E, 650, 300, 700, 350}, /* Up */
    {0x0D, 650, 400, 700, 450}, /* Down */
    {0x0A, 100, 350, 150, 400}, /* Bullet */
};
...
MainScreen(void)
{
    ...
    // if(ffIndex < 20)
        // frameFIll[ffIndex++] = DMTimerCounterGet(SOC_DMTIMER_2_REGS)
        ...
        CacheDataCleanBuff((unsigned int)g_pucBuffer0, (unsigned int)(800*480*4) + 32);
        CacheDataCleanBuff((unsigned int)g_pucBuffer1, (unsigned int)(800*480*4) + 32);
...
extern unsigned int xdata;
extern unsigned int ydata;
...
void getInputs(void)
{
    ...
    if(IsTSPress)
    {
        ...
        {
            if((xdata >= keys[i_index].x_min) && (xdata <= keys[i_index].x_max) &&
              ((ydata >= keys[i_index].y_min) && (ydata <= keys[i_index].y_max)))
            {
...
void updateFrameBuffer()
{
    if(0 == frameToggle)
    {
        while(1 != flagA);    // should be updated when the frame is not transferred
